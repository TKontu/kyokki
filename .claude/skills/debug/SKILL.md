...
Resolve the test command from the **Project Commands** table in `CLAUDE.md` (between the
`claude:commands` markers) — `test-one` while iterating, `test` before you call it fixed.
...
- Question the assumption underneath all three — they probably shared one
- Say what you have ruled out, and ask the operator (or your orchestrator, if you have one) before
  trying a fourth
